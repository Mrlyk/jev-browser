// jev-browser fork: isolate persisted state/configuration and internal binary tests.
...
/// Priority: AGENT_BROWSER_SOCKET_DIR > XDG_RUNTIME_DIR > ~/.jev-browser > tmpdir
...
pub fn send_command(cmd: Value, session: &str) -> Result<Response, String> {
    // jev-browser: never replay a command after dispatch when the wrapper opts in.
    if std::env::var("AGENT_BROWSER_NO_REPLAY").as_deref() == Ok("1") {
...
pub fn daemon_unreachable(error: &str) -> bool {
    if error.starts_with("EXECUTION_UNKNOWN:") {
        return false;
    }
...
fn send_command_once(cmd: &Value, session: &str) -> Result<Response, String> {
    ...
    stream
        ...
        .map_err(|e| dispatch_error(format!("Failed to send: {}", e)))?;

    let mut reader = BufReader::new(stream);
    let mut response_line = String::new();
    reader
        .read_line(&mut response_line)
        .map_err(|e| dispatch_error(format!("Failed to read: {}", e)))?;

    serde_json::from_str(&response_line)
        .map_err(|e| dispatch_error(format!("Invalid response: {}", e)))
}

// After attempting a write, partial dispatch is possible. Do not reconnect/replay.
fn dispatch_error(message: String) -> String {
    if std::env::var("AGENT_BROWSER_NO_REPLAY").as_deref() == Ok("1") {
        format!("EXECUTION_UNKNOWN: {}", message)
    } else {
        message
    }
}
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn jev_dispatch_disconnect_is_unknown_and_never_replayed() {
        ...
        let guard = EnvGuard::new(&[
            ...
            "AGENT_BROWSER_NO_REPLAY",
        ]);
        let dir = tempfile::tempdir().unwrap();
        guard.set("AGENT_BROWSER_SOCKET_DIR", dir.path().to_str().unwrap());
        guard.remove("AGENT_BROWSER_NAMESPACE");
        guard.set("AGENT_BROWSER_NO_REPLAY", "1");
        let listener = UnixListener::bind(get_socket_path("jev-replay-test")).unwrap();
        ...
        let server = thread::spawn(move || {
            while !server_stop.load(Ordering::SeqCst) {
                if let Ok((stream, _)) = listener.accept() {
                    ...
                    // The daemon received the operation but died before its response.
                } else {
        ...
        server.join().unwrap();
        let error = result.err().expect("dispatch must fail after disconnect");
        assert!(error.starts_with("EXECUTION_UNKNOWN:"), "{}", error);
    ...
    fn test_get_socket_dir_ignores_empty_socket_dir() {
        ...
        assert!(get_socket_dir().to_string_lossy().ends_with(".jev-browser"));
    }
    ...
    fn test_get_socket_dir_home_fallback() {
        ...
        assert!(result.to_string_lossy().ends_with(".jev-browser"));
        assert!(
