...
Plugins are configured in jev-browser.json. A plugin entry declares a name,
...
Add options:
  ...
  --global                 Write ~/.jev-browser/config.json instead of ./jev-browser.json
...
Configuration:
  agent-browser looks for jev-browser.json in these locations (lowest to highest priority):
    1. ~/.jev-browser/config.json      User-level defaults
    2. ./jev-browser.json              Project-level overrides
