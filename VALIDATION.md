...
基线为 `2f7a6ad`，对照组仅将 Key 前缀规则修正为 `sk`，实验组再删除登录流程、帮助和使用文档中的内部判断及检查说明。三组使用相同的 synthetic Key、固定模型响应和浏览器执行桩；通过真实 PTY 输入，不请求真实模型或启动浏览器。

| 组别 | 新规则识别通过 | 手动登录输出字符数 | 自动登录及原命令输出字符数 |
| --- | --- | --- | --- |
| 原版 | 2/8 | 111 | 135 |
| 仅修正前缀 | 8/8 | 113 | 137 |
| 修正前缀并精简文案 | 8/8 | 21 | 45 |

字符数来自相同 `sk-test` 输入下的 stdout 与 stderr，包含换行；自动登录还包含执行桩的固定完成提示。原版的失败由旧前缀规则导致。
