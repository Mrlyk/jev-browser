...
Native atomic errors can use a different envelope, including a string `error` and top-level `code`. The semantic `page act` error schema must not be assumed for every command.
...
- **Browser download/launch failure:** the first `page open` can prepare Chrome. Check network, certificate, directory permissions, and browser system libraries. Linux ARM64 needs an installed Chromium via `--executable-path`; a host sandbox may require an outside-sandbox browser and CDP.
...
```bash
jev-browser skill list
jev-browser skill get jev-browser
jev-browser skill path jev-browser
```

The default `get` returns only the main guide. Read the referenced file from the returned folder when needed. `skill get jev-browser --full` explicitly includes all four reference files.
